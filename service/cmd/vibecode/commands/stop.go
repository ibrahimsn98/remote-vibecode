...
var StopCmd = &cobra.Command{
	...
	Short: "Stop an rvc session",
	Long:  `Stop (kill) an rvc tmux session. You'll be prompted to confirm unless --force is used.`,
	RunE:  runStop,
}
...
func runStop(cmd *cobra.Command, args []string) error {
	...
	if !tmux.SessionExists(sessionName) {
		return fmt.Errorf("session '%s' does not exist. Use 'rvc list' to see available sessions.", sessionName)
	}
